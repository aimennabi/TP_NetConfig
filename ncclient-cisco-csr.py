...
def print_config(manager):
    ...
    config = get_config(manager,datastore)
    print(config)
...
def print_config_ietf_if(manager):
    ...
    config_ietf_if = get_config(manager,datastore, ('subtree', IETF_INTERFACES) )
    print(config_ietf_if)
...
def print_config_cisco_if(manager):
    ...
    config_cisco_if = get_config(manager,datastore, ('subtree', CISCO_INTERFACES) )
    print(config_cisco_if)
...
def commit_confirmed(manager):
    ...
    try:
        reply = manager.commit(confirmed=True, timeout='60')
        if reply.ok:
            print("commit --> OK")
        else:
            print("commit --> ERROR")
